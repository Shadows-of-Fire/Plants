...
public class BlockDoubleMetaBush extends BushBase implements IMetaPlant {
	...
	public IBlockState getStateFromMeta(int meta) {
		int k = meta % 2;
		if (k == 0)
			return this.getDefaultState().withProperty(UPPER, true).withProperty(META, getActualMeta(meta));
		if (k == 1)
			return this.getDefaultState().withProperty(UPPER, false).withProperty(META, getActualMeta(meta));
		else
			return null;
	}
