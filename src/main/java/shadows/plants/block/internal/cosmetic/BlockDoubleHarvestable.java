...
public class BlockDoubleHarvestable extends BlockHarvestable{
	 ...
	 public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, @Nullable ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ){
		if(player.getHeldItemMainhand() == Data.EMPTYSTACK){
			if(state.getValue(FRUIT) && state.getValue(UPPER)){
				...
				world.setBlockState(pos.down(), getDefaultState().withProperty(UPPER, false).withProperty(FRUIT, false));
				return true;
			}
    ...
    protected void checkAndDropBlock(World world, BlockPos pos, IBlockState state)
    {
        if (!this.canBlockStay(world, pos, state)){
            ...
            if (!flag) this.dropBlockAsItemWithChance(world, pos, state, 1.0F, 0);;
    ...
    {
    	world.setBlockState(lowerPos, this.getDefaultState().withProperty(UPPER, false).withProperty(FRUIT, false), flags);
    	world.setBlockState(lowerPos.up(), this.getDefaultState().withProperty(UPPER, true).withProperty(FRUIT, false), flags);
    }
    ...
    public boolean removedByPlayer(IBlockState state, World world, BlockPos pos, EntityPlayer player, boolean willHarvest)
    {
	 ...
	 {
		 if(meta2 == 0) return getDefaultState();
		 if(meta2 == 1) return blockState.getBaseState().withProperty(UPPER, false).withProperty(FRUIT, false);
		 if(meta2 == 2) return blockState.getBaseState().withProperty(UPPER, true).withProperty(FRUIT, true);
		 if(meta2 == 3) return blockState.getBaseState().withProperty(UPPER, false).withProperty(FRUIT, true);
		 else return null;
	 }
	 ...
	 {
		 int k = 0;
		 if (!state.getValue(UPPER)) ++k;
		 if (state.getValue(FRUIT)){ ++k; ++k; }
		 return k;
	 }
